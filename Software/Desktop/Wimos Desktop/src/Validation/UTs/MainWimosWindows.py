...
class QMainWimosWindows(QMainWindow):
    ...
    def TestUTs(self):
        print("UTs Starting...")
        TimerToTest = QTimer(self)
        TimerToTest.setSingleShot(True)
        TimerToTest.timeout.connect(self.MainWindows.TestUTs)
        TimerToTest.start(5000)
        
    def TestVT(self):
        print("VT Starting...")
        TimerToTest = QTimer(self)
        TimerToTest.setSingleShot(True)
        TimerToTest.timeout.connect(self.MainWindows.TestVT)
        TimerToTest.start(5000)
...
def main():
    ...
    if (sys.argv[-1] == "UTs"):
        print("[TEST] Unitary Test Mode")
        w.TestUTs()
    elif (sys.argv[-1] == "VT"):
        print("[TEST] Validation Test Mode")
        w.TestVT()
